...
VEHICLE_TYPE_RATES     = {t['vehicle_type']:   t              for t in VEHICLE_TYPE_MAPPING}
...
def generate_uber_ride_confirmation(vehicle_type: str = None) -> dict:
    """
    Generate a realistic Uber ride confirmation record.

    Args:
        vehicle_type: Optional ride type to fix (e.g. 'UberX'). Random if not provided.

    Returns:
        dict with all ride fields matching the DB schema.
    """
    # Timestamps
    ...
    # Vehicle / ride type
    vt = vehicle_type if vehicle_type in VEHICLE_TYPE_RATES else random.choice(VEHICLE_TYPES_LIST)
    rates = VEHICLE_TYPE_RATES[vt]
    ...
    subtotal         = round((distance_fare + time_fare + rates['base_rate']) * surge_multiplier, 2)
    tip              = round(random.choice([0, 0, 0, 1, 2, 3, 5, round(random.uniform(1, 20), 2)]), 2)
    ...
    return {
        ...
        # Foreign keys
        'vehicle_type_id':          VEHICLE_TYPE_ID_MAP[vt],
